...
package body Assgn is 
   ...
   begin 
      for i in reverse Reverse_Bin_Array'range loop
         if Quotient = 0 then 
            remainder := 0;
         else 
         ...
         Reverse_Bin_Array(i) := BINARY_NUMBER(Remainder);
      end loop;
      return Result_Bin_Array;
   end Int_To_Bin;
   
   funtion Bin_To_Int (Arr : in BINARY_ARRAY) return INTEGER is 
      Multiplier: Ineger := 1;
      Result: Integer := 0;
